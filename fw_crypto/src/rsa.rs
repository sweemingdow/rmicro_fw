...
impl Rsa {
    pub fn from_str(
        pri_key: &str,
        pub_key: &str,
        display_type: RsaKeyDisplayType,
    ) -> FwResult<Self> {
...
pub fn gen_rsa_key_pair_as_b64(bits: u16) -> FwResult<(String, String)> {
    let (pri_key, pub_key) = gen_rsa_key_pair(bits);
...
pub fn gen_rsa_key_pair_as_pem(bits: u16) -> FwResult<(String, String)> {
    let (pri_key, pub_key) = gen_rsa_key_pair(bits);
...
fn gen_rsa_key_pair(bits: u16) -> (RsaPrivateKey, RsaPublicKey) {
    let mut rng = thread_rng();
...
mod tests {
    use crate::rsa::{
        Rsa, RsaKeyDisplayType, decrypt, encrypt, gen_rsa_key_pair_as_b64, gen_rsa_key_pair_as_pem,
    };

    #[test]
    fn test_gen_key() {
        let (pri_key, pub_key) = gen_rsa_key_pair_as_b64(2048).unwrap();
        println!("pri_key={pri_key}");
        println!("pub_key={pub_key}");

        let (pri_key, pub_key) = gen_rsa_key_pair_as_pem(2048).unwrap();
        println!("pri_key={pri_key}");
        println!("pub_key={pub_key}");
    }
    ...
    fn test_rsa_encrypt_decrypt() {
        let (pri_pem, pub_pem) = gen_rsa_key_pair_as_pem(2048).unwrap();
        ...
        let (pri_pem, pub_pem) = gen_rsa_key_pair_as_b64(2048).unwrap();
        let plain = "我是大哥啊@fdfdsf_*&^$#iewj~fdlsjl---------~33!- - ~ -~@!@!!!!!!fdsf火星文誃尐亽籟萿亍丗，萿嘚像嗰怎庅說嘟卟嗵の徣ロ";
    ...
    fn test_rsa_struct() {
        ...
        let (pri_pem, pub_pem) = gen_rsa_key_pair_as_b64(4096).unwrap();
